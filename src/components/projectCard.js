...
// import { pushDataToFirestore, sampleData } from "../utils/firebase";
...
import {
  askRefundFromProject,
  claimFundsFromProject,
  deleteProject,
            ...
            <MDBCardBody>
              <MDBRow className="align-items-baseline justify-content-end p-0 m-0">
                <MDBCol md="6" className="p-0 m-0 pe-1">
                  <MDBCardTitle className="text-light fs-4">
                    {project.name}
                  </MDBCardTitle>
                  <MDBCardSubTitle className="text-light text-muted">
                    Goal: {microAlgosToString(project.goal)} ALGO
                  </MDBCardSubTitle>
                </MDBCol>
...
function ProjectCard({ project, action, address, ...props }) {
  ...
  const lightButtonAction =
    action === "claim" ? deleteThisProject : goToProject;
  const lightButtonText = action === "claim" ? "Delete" : "Info";

  const projectCannotBeDeleted = useCallback(() => {
    return (
      (project.deleted && action === "claim") ||
      (action === "claim" && Date.now() / 1000 < project.endDate)
    );
  }, [project, action]);
              ...
              <MDBRow className="align-items-baseline justify-content-end pt-3 m-0">
                <MDBCol md="4" className="text-center pb-2">
                  <Button
                    ...
                    disabled={projectCannotBeDeleted()}
                    onClick={lightButtonAction}
                  >
                    {lightButtonText}
                  </Button>
                </MDBCol>
...
function ProjectActionButton({ action, project, address }) {
  const isRefundOk = useCallback(() => {
    return (
      Date.now() / 1000 > project.endDate &&
      project.current_amount < project.goal &&
      !project.deleted
    );
  }, [project]);
  ...
  return action === "donate" ? (
    <DonateButton project={project} sender={address}>
      Donate
    </DonateButton>
  ) : action === "refund" ? (
    <Button
      ...
      style={{ minWidth: "130px" }}
      disabled={!isRefundOk()}
      onClick={() => askRefundFromProject(address, project)}
    >
      Ask Refund
    </Button>
  ) : (
    <Button
      ...
      style={{ minWidth: "130px" }}
      disabled={!isClaimOk()}
      onClick={() => claimFundsFromProject(address, project)}
    >
      Claim Funds
    </Button>
  );
...
export function ProjectList({
  ...
  return (
    ...
    >
      {useTitle ? (
        <h3
          className="text-center font-title text-light"
          style={{ fontSize: "40px" }}
        >
          <b>All our projects</b>
        </h3>
      ) : (
        <></>
      )}
...
export function ProjectListNotLoggedIn({ projects }) {
  ...
  //   console.log(`tried to push`, sampleData);
  ...
  return (
    ...
    >
      <h3
        ...
        style={{ fontSize: "40px" }}
      >
        {/* <Button onClick={handleDataPush}>Push Data to Firestore</Button> */}
        <b>All our projects</b>
      </h3>
      {projects.map((project, index) => (
        <div key={index}>
